...
local prevSpeed
local prevTime
local speedAnnVersion = "1.1"

--------------------------------------------------------------------------------
-- Read and set translations
local function setLanguage()
    local lng=system.getLocale()
    local file = io.readall("Apps/SpeedSp/config.json")
...
local function initForm()
    ...
    if(fw >= 4.22)then
        ...
        addRow(1)
        addLabel({label="Speed Speech - v."..speedAnnVersion.." ", font=FONT_MINI, alignRight=true})
        else
...
local function loop()
    ...
    if (swi and swi == 1) then
		if(sensor and sensor.valid) then
		  ...
		  if(spUnit==5) then value=value*factor*1.94384 end
...
setLanguage()
collectgarbage()
return {init = init, loop = loop, author = "DM", version = speedAnnVersion, name = trans11.appName}